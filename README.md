...
* Layers: groups, drill-in, reorder, rename, eye / lock, search filter with auto-expanded groups, context menu (copy / paste / duplicate / group / arrange / delete, hides inapplicable rows), canvas align + distribute, animation clip copy / paste across shapes and designs, Figma-style masks (any shape masks siblings above, stacked bands, nested intersection) with animated wipe / iris reveals
...
* Design panel with mixed-value handling, typography controls, a Figma-style color picker, and three-dot copy / paste of design values, properties or both across shapes and designs
* Dark / light theme with Appearance settings (per-theme colors, canvas + window chrome, corner presets, UI font with custom file imports, tab rail position), theme-aware Bibata mouse cursors (Ice for light, Classic for dark), custom frameless titlebar with draggable tabs (titles follow design names, dockable top / bottom / left / right with collapsible side rails, hideable chrome)
* Editable shortcuts: remappable with search filter, duplicate blocking, per-row undo and Reset all
* General settings: new-design defaults (canvas size presets, background, timeline length), video-export defaults (quality, frame rate, effort, format) and storage footprint
* Plugins: QML-only drop-in folders with permission approval and sandbox (no remote loading), toolbar / design / appearance / canvas / full-overlay slots, mediated image-audio picks plus scoped storage — Official plugins carry a checksum-verified seal and the bundled theme pack ships eighteen light + dark presets in Settings → Appearance via one-click `appearance.write` (stays tweakable after)
